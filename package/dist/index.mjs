...
const getDeployments = async (accessToken, teamId) => {
  const url = `https://api.vercel.com/v6/deployments` + (teamId ? `?teamId=${teamId}` : "");
...
const getDeploymentFiles = async (accessToken, teamId, deployment) => {
  const url = `https://api.vercel.com/v7/deployments/${deployment.uid}/files` + (teamId ? `?teamId=${teamId}` : "");
  const response = await fetch(url, {
    headers: {
      Authorization: `Bearer ${accessToken}`
    },
...
const checkDeploymentSource = (deployment) => {
...
const fileContentFetcher = (accessToken, teamId, deployment) => {
  return async function(file) {
    const url = `https://api.vercel.com/v7/deployments/${deployment.uid}/files/${file.uid}` + (teamId ? `?teamId=${teamId}` : "");
    const response = await fetch(url, {
      headers: {
        Authorization: `Bearer ${accessToken}`
      },
...
const downloadFile = async (entry, path, fetcher) => {
...
const parseFileTreeEntry = async (entry, path, fetcher) => {
...
const parseFileTree = async (fileTree, path, fetcher) => {
  for (let entry of fileTree) {
    await parseFileTreeEntry(entry, path, fetcher);
  }
};
const countFileTreeLength = (fileTree) => {
  let count = 0;
  for (let entry of fileTree) {
...
async function init() {
  ...
  const chooseDeployment = await prompts({
    ...
    choices: deployments.map((deployment2) => {
      const age = dayjs(deployment2.ready).from(dayjs());
      return {
        title: deployment2.url,
        description: `${deployment2.target} (${age})`,
        value: deployment2
      };
    })
  });
  const deployment = chooseDeployment.value;
